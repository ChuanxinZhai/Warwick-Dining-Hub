...
def facilities():
    ...
    if facility_query:
        # When there is a specific facility parameter, only products from that facility are acquired
        products = conn.execute('SELECT * FROM product WHERE facility = ?', (facility_query,)).fetchall()
        facilities_info = {facility_query: products}
    else:
        # Get products from all merchants when there is no specific facility parameter.
        facilities_info = {}
